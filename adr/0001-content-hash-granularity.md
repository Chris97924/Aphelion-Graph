...
Aphelion must define what a "content hash" identifies. Two candidate granularities were considered:
...
- Downstream consumers (Parallax and others) depend on knowing whether Aphelion owns cross-source identity.

## Decision

**Aphelion adopts per-claim SHA-256 as the single content-hash granularity.**

1. The hash of a claim is the SHA-256 of its canonical serialized bytes (see `spec/canonical-serialization.md`).
2. The hash of the package is the SHA-256 of the canonical `*.aphelion.tar` byte stream (see `spec/packaging.md`).
3. **Cross-source aggregation is explicitly out of scope for the Aphelion format.**
   - Merging, deduplicating, or re-attesting claims across different source packages is an **application-layer concern**.
   - Parallax, Cmemory, or any downstream consumer MAY define their own aggregate identity, but such aggregates are NOT Aphelion constructs and MUST NOT be encoded inside the package.
...
- **Composability**: applications can mix claims from multiple packages without Aphelion-level re-hashing.
- **Spec minimality**: one hash rule, one granularity — no "which hash?" ambiguity.

### Negative

- **No built-in dedup**: if the same logical claim appears in two packages, Aphelion cannot natively detect it. Consumers that need dedup must implement their own canonicalization.
