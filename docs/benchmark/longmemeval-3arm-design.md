...
4. **20 adversarial questions.** A held-out set of question types where
   ...
   **Adversarial rule (pre-registered — §4 row AG):** C − B ≤ +3 pp on the 20
   ...
   as two tiers (amended 2026-08-14, §4 row AG): Tier 1 at `C − B` = +5pp is a
   bounded single-question context diff; Tier 2 at `C − B` ≥ +10pp is the full
   leakage investigation. This checks that C wins for the *right reason*.
