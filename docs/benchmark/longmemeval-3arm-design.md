...
**Amended:** 2026-08-16 — §7.3 vocabulary-primed extraction, recorded *before* any arm has run. The 2026-08-15 structured-extraction amendment worked: a 2026-08-16 probe over 10 knowledge-update questions produced **11 `supersedes` edges across 4 of them**, where the previous mechanism produced zero everywhere. But **6 questions still produced none, and every miss was a naming difference rather than a missing fact** — question `01493427` called one fact `user/postcard-collection/new-acquisitions-count` (17) in one session and `user/collection/postcards/new-additions-since-restart` (25) in the next. Each session's extraction is an independent call, so the model cannot reuse slugs it minted an hour of corpus earlier. Within a question, every session after the first is now shown its predecessors' subject slugs in a fenced `KNOWN_SUBJECTS` block and instructed to reuse them verbatim for the same fact, minting a new slug only for a genuinely new one. The vocabulary is scoped per question and derived from the sessions *preceding* the one being extracted in pinned order, so it stays a pure function of the frozen sequence, one shared extraction still feeds all three arms, and a resumed run rebuilds it from cached claims and sends the prompts the original sent. **No gate, threshold, denominator or model pin moves** — this changes only what the shared extract stage is told (§6.3).
...
power is *not* bounded by the 78-question knowledge-update pool. **M3 runs on the
72 non-abstention knowledge-update questions** (amended 2026-08-14; the 6 `_abs`
variants are excluded — see the §4 M3 row) — old→new value labels exist only for
knowledge-update questions, and only for those that actually carry an update — so
M3 carries the same CI caveat as M1; it is nonetheless
...
|---|---|---|---|
| **M1** | QA accuracy on knowledge-update | **C − B ≥ +3 pp** | The honest test is C beating the *naive-dedup* control, not plain A. +3pp is the v0.2 minimum improvement deemed worth the state-machine complexity. **Caveat:** N = 78 → ≈ 2.3 questions; report a bootstrapped CI and treat as directional (§3.3). Report C − A as secondary context. **Amendment 2026-08-14 (§6.3) — interpretation added, threshold unchanged:** `+3pp` is a pre-registered **decision rule, not a significance test**. At N = 78 the smallest paired difference that can reach p < 0.05 is 6 net discordant questions (7.7pp), so a bootstrapped CI around a +3pp effect will essentially always straddle zero. The CI is therefore **reported for honesty and does not overturn the gate in either direction** — it can neither rescue a C − B below +3pp nor veto one at or above it. Pinning that reading now closes the hole the original text left open: "report a bootstrapped CI" without saying what a straddling CI *means* is an interpretation chosen after seeing the result, which is exactly what §6.3 forbids. |
...
| **M3** | Stale-info contamination rate (denominator N = 72 non-abstention KU) | **C ≤ 0.5 × A** | `superseded`/`withdrawn` suppression should at least halve the rate at which a stale (superseded) value appears in the retrieved context, vs plain storage. Contamination = fraction of the **72 non-abstention knowledge-update** answers whose retrieved context contains the *old* value — old→new value labels exist only for that subset, and only where an update actually exists (same CI caveat as M1; §3.3). Multi-session questions are **not** a current M3 substrate (no stale-value labels); extending M3 to them is labeled future work requiring stale-value annotation, not current measurability. **Amendment 2026-08-14 (§6.3) — denominator corrected 78 → 72, ratio unchanged:** 6 of the 78 knowledge-update questions are abstention variants (`_abs`: `031748ae_abs`, `0ddfec37_abs`, `2133c1b5_abs`, `2698e78f_abs`, `6aeb4375_abs`, `f685340e_abs`) whose gold answer is *"The information provided is not enough…"*. They encode no old→new update, so **no stale-value label can exist for them** — under N = 78 they sat in the denominator as 6 structurally-uncontaminable questions, identical for every arm, deflating A and C alike and making the reported rate not mean what this row says it means. The pinned justification ("labels exist only for that subset") *requires* the exclusion; keeping 78 contradicted it. The `C ≤ 0.5 × A` ratio is untouched. **Amendment 2026-08-14 (§6.3) — INCONCLUSIVE rule added, ratio unchanged:** `C ≤ 0.5 × A` is scale-free, so whether a verdict is *readable* depends on how much evidence actually separates the arms — which nobody knows before the run. M3 scores every arm against the same 72 questions, so A and C are **paired**: let **b** = questions contaminated in A but not in C, and **c** = questions contaminated in C but not in A. Concordant questions (contaminated in both arms, or in neither) carry no information about which arm is better and must not count as evidence. Readability is therefore decided by the **exact two-sided sign test on the discordances**: with `n = b + c`, `p = min(1, 2 × P(X ≤ min(b, c)))` for `X ~ Binomial(n, ½)`. **If `p ≥ 0.05` M3 is INCONCLUSIVE — not pass, not fail — and does not fire §8's "demote the event state machine" branch, nor count toward §8's "All pass" row.** Only at `p < 0.05` is the pinned `C ≤ 0.5 × A` ratio read for pass/fail. The ratio itself is untouched; this rule only decides whether reading it is warranted. **Worked example of why a marginal count is the wrong quantity:** A = 12, C = 7 with all seven of C's contaminated questions also contaminated in A gives `b = 5, c = 0, n = 5, p = 0.0625` — statistically unreadable, even though A's raw count is high and the ratio is missed. A rule keyed on A's count alone would have called that readable and fired the §8 demotion on five discordant questions. Pinned in advance so a corpus that cannot separate the arms yields neither a spec retreat nor a win on noise. **Amendment 2026-08-15 (§6.3) — label source and matching rule pinned, denominator corrected 72 → 66, ratio unchanged:** M3 had an unmet precondition of its own. LongMemEval ships **no old-value annotation**, so until now there was no way to score this row at all, and the tempting shortcut — deriving old values from the harness's own shared-linker `supersedes` edges — was **refused**, because those are precisely the edges Arm C acts on and the metric would have measured Arm C against its own mechanism, driving its contamination toward zero by construction. Labels are therefore supplied as **data**: `benchmarks/longmemeval/m3_labels.json`, pinned by sha256, produced on 2026-08-15 by `qwen3.6:latest` at temperature 0 — a model that is neither the pinned answering nor the pinned extractor model and is never invoked by `benchmarks/longmemeval` — with every candidate mechanically verified to be a verbatim substring of its own question's evidence sessions, to differ from the gold answer, and to sit inside a quote that is itself a corpus substring; nothing the labeling model asserted is trusted on its word (`docs/benchmark/m3-labels-methodology.md`). **Matching is pinned as token-boundary and case-sensitive**, `(?<!\w)`·`re.escape(value)`·`(?!\w)`, superseding raw substring containment: 23 of the 70 labels are ≤ 4 characters ("4", "20", "two"), so substring matching fires on "42", "2024" and "14:30", and since this gate is a **ratio** the same false-positive mass lands on both `C` and `A` and pushes `C / A` toward 1 — biasing the pinned gate toward FAIL rather than merely adding noise. Lookarounds are used rather than `\b` because many labels begin or end with a non-word character (`$350`, `3-2`, `7:00 pm`), which `\b` cannot anchor. **The denominator moves 72 → 66** on the same factual-correction ground as the `_abs` step: the labeling pass read all 72 transcripts in full and found 6 questions (`0977f2af`, `10e09553`, `22d2cb42`, `5c40ec5b`, `9bbe84a2`, `dfde3500`) with no superseded value anywhere in their evidence — four *ask for* the earlier value, so their gold answer **is** the historical fact and nothing supersedes it, and two state their value once and never revise it. This row's own justification already restricts M3 to questions "where an update actually exists", so keeping them contradicted it exactly as N = 78 did: they are uncontaminable for every arm alike, deflating A and C by the same 6 questions. The labels file still carries all 72 structural keys (the 6 as empty lists) so the exclusion stays auditable. **`C ≤ 0.5 × A` is untouched** — a common denominator cancels — as are α = 0.05 and the sign test, and no arm has run. |
...
| **AG** | Adversarial-set advantage (bias guard §6, item 4) | **diagnostic tripwire (non-gating): C − B ≤ +3 pp on the 20 adversarial questions** | New rule, pinned 2026-07-19 (v0.2 named the adversarial set but pinned no number). On questions where aphelion's machinery structurally cannot help, C must not gain more than the very margin M1 requires it to gain on the real set; a larger adversarial gain signals arm-identity leakage or spurious signal. Non-gating because N = 20 (+3pp ≈ 0.6 questions cannot support a hard pass/fail); a breach **mandates** a leakage investigation before M1/M3 are trusted. Pinned here so the rule is fixed before any run. **Amendment 2026-08-14 (§6.3) — breach response tiered, tripwire unchanged:** at N = 20 one question is 5pp, so the smallest non-zero value `C − B` can take already breaches `≤ +3pp` — the tripwire's operative threshold is "C must not be ahead by even one question," and under a true null (C and B equally likely to win any discordant question) it fires roughly **25–36%** of the time. A mandated full investigation at a one-in-three false-alarm rate would become routine and be performed perfunctorily, defeating the guard. The `+3pp` number is **kept** (moving a bias guard's threshold before a run it was written to constrain is the wrong optic); what is pinned here is the *depth* of the mandated response, fixed in advance per §6 guard 3: **Tier 1 — `C − B` = +5pp (a one-question *net* advantage):** enumerate **every discordant adversarial question — both the B-only wins and the C-only wins — and diff each one's Arm B vs Arm C retrieved context**, recording all findings in the results. A net +5pp does **not** mean exactly one question differs: three C-only wins against two B-only wins nets the same +5pp, so there is no unique "the differing question" to inspect, and picking one arbitrarily could miss the very question carrying the leakage while still licensing M1/M3 to be trusted. At N = 20 the discordance set is small by construction, so full enumeration stays a bounded check rather than an audit. **Tier 2 — `C − B` ≥ +10pp:** the full leakage investigation, completed before M1/M3 are trusted, per §6 guard 4. |
...
4. **20 adversarial questions.** A held-out set of question types where
   ...
   **Adversarial rule (pre-registered — §4 row AG):** C − B ≤ +3 pp on the 20
   ...
   trusted. **The depth of that mandated investigation is itself pre-registered**
   as two tiers (amended 2026-08-14, §4 row AG): Tier 1 at a `C − B` = +5pp net
   advantage is a bounded pass over **every** discordant question — B-only and
   C-only alike, since a one-question net does not imply a single discordance;
   Tier 2 at `C − B` ≥ +10pp is the full leakage investigation. This checks that
   C wins for the *right reason*.
...
**Amendment 2026-08-16 (§6.3) — vocabulary priming closes the residual drift.**
Structured extraction lifted the ceiling off the floor but not all the way. A
probe on 2026-08-16 over 10 knowledge-update questions found **11 `supersedes`
edges on 4 questions** — the mechanism is alive — while **6 produced none, every
miss a naming difference for a fact whose value had plainly moved**:

| Question | Session 1 slug (value) | Session 2 slug (value) |
|---|---|---|
...
The cause is structural, not a wording failure: **each session's extraction is an
independent call**, so the model has no way to reuse a slug it minted for the
same fact earlier in the same question. No rubric phrasing fixes that, because
the model is not being inconsistent within a call — it is being asked twice, in
isolation, to name the same thing.

Within a question, sessions already run in pinned occurrence order. After each
session's extraction the question's subject vocabulary is accumulated — slug plus
the latest value seen, in first-minted order — and every **subsequent** session's
prompt carries it as a fenced `KNOWN_SUBJECTS` block with one instruction: when a
claim concerns a listed fact, reuse that slug character-for-character; mint a new
slug only for a fact not listed. The first session of a question is unprimed.

Three properties are preserved deliberately. The vocabulary is **per question**,
so nothing leaks between questions. It is derived from the sessions *preceding*
the one being extracted rather than from everything seen so far, so it is a pure
function of the pinned sequence and not of how many arm passes have replayed it —
which is also what lets a resumed run rebuild it from cached claims and send the
prompts the original run sent. And it primes **one shared extraction** that feeds
all three arms, so Arm C is handed nothing Arms A and B are not.
...
| **M3 fail** | `superseded`/`withdrawn` suppression not reducing contamination. | **Demote the event state machine** — question whether `supersede`/`withdraw` belong in the retrieval-surfacing path at all. **Amendment 2026-08-14 (§6.3):** this row fires only on a *readable* M3. Per the §4 M3 INCONCLUSIVE rule, if the exact two-sided sign test over the paired A-only / C-only contamination discordances yields `p ≥ 0.05`, M3 is **INCONCLUSIVE — neither pass nor fail** — so it neither triggers this demotion nor counts toward the "All pass" row above; the honest action is to report `b`, `c`, the raw per-arm counts and the exact p, and note that the corpus could not separate the arms. |
