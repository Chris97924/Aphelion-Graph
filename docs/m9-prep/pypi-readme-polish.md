...
**Aphelion** 是一套開源決策紀錄格式與工具鏈，讓每一個決策都有跡可循、可驗證、可攜帶。不同於散落在 Wiki、Slack thread 或 PDF 裡的 ADR，Aphelion 將決策結構化為可機器驗證的 canonical document，搭配八支 CLI 子命令（`init` / `validate` / `pack` / `unpack` / `diff` / `migrate` / `verify` / `sign`）與三層 validator，確保從草稿到簽署的每一筆變更都可被獨立 audit。
...
Requires Python 3.10+. Core dependencies: standard library only. Optional `aphelion[signer]` extra adds `cryptography>=42` for Ed25519 signing.

> CLI 進入點為 `aphe`（見 `pyproject.toml` `[project.scripts]`）。下方 quickstart 示範直接呼叫 `aphe <subcommand>`。
...
aphe validate ./ADR-0042/

# 3. 打包為可分發的 canonical bundle
aphe pack ./ADR-0042/ ADR-0042.aphelion.tar

# 4. 解包收到的 bundle
aphe unpack ADR-0042.aphelion.tar ./restored/

# 5. 比較兩個解開後的版本（claim-level diff）
aphe diff ./ADR-0042/ ./restored/

# 6. 驗證 archive 結構與 hash 完整性（structural + content-hash 路徑）
aphe verify ./ADR-0042.aphelion.tar
# 若已有 signer envelope，加 --require-signed 進入 signature + trust-chain 驗證
# aphe verify --require-signed ./ADR-0042.aphelion.tar
```
...
|---|---|---|
| **M9** | PyPI publish (`aphelion`) | 🔄 In progress |
...
- **Implementations in other languages** — We actively encourage ports. See `docs/m9-prep/implementor-guide.md` for serialization rules, compliance checklist, and golden-fixture pointers.
