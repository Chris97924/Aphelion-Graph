...
Two prefix namespaces are reserved for application-defined extensions. DPKG
...
1. **Preservation.** DPKG packers and unpackers MUST round-trip reserved-prefix
...
3. **Diff.** `dpkg diff` MUST NOT report differences in reserved-prefix keys
...
5. **Exchange profile.** The set of semantically meaningful reserved keys for
   ...
   `spec/parallax-exchange.md` in the Parallax repo). DPKG has no view into
   them.
...
without affecting the canonical DPKG identity surface. Reserving fixed
